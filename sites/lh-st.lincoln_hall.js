...
const logger = require("../support/logger.js")("LINCOLN_HALL");
...
function transform(html, preEvent) {
  ...
  const events = $(".tessera-card-deck .card")
    ...
    .map((item) => {
      ...
      if (venueName?.toLocaleLowerCase().includes("schubas")) {
        return;
      }
...
async function main() {
  ...
  // todo: this etl can be merged with lh-st.js, it's basically the same
